...
int Socket::sendsome(
    ...
    *was_closed = false;
     ...
     * (ver más abajo).
     * */
    int s = send(this->fd, data, sz, MSG_NOSIGNAL);
    if (s == -1) {
        /*
...
Socket Socket::accept() {
     ...
     * (`this->fd`) para seguir haciendo más llamadas a `accept`
     * independientemente de que enviemos/recibamos del socket `peer`.
     * */
    int peer_skt = ::accept(this->fd, nullptr, nullptr);
    if (peer_skt == -1)
