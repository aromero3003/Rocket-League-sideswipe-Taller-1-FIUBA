...
MenuProtocol::MenuProtocol(Socket&& o_skt, GameHandler& gameRef)
    : skt(std::move(o_skt)), was_closed(false), games(gameRef) {}

Socket& MenuProtocol::getSocketRef() { return this->skt; }
  ...
  try {
    while (!was_closed) {
      ...
      std::unique_ptr<Command> command(
          commandHandler.createCommand(reciveCommand(), skt));
      this->was_closed=this->skt.isClosed();
      if (!was_closed) {
